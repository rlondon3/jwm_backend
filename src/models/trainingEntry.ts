...
export class TrainingJournalStore {
	async getEntriesByUserId(user_id: number): Promise<TrainingEntry[]> {
		try {
			const sql =
				'SELECT * FROM training_entries WHERE user_id=($1) ORDER BY practice_date DESC;';
	...
	async getEntryById(id: number): Promise<TrainingEntry> {
		try {
	...
	async create(entry: TrainingEntry): Promise<TrainingEntry> {
		try {
			const conn: PoolClient = await client.connect();
			const sql = `
	...
	async update(entry: TrainingEntry): Promise<TrainingEntry> {
		try {
			const sql = `
	...
	async delete(id: number, user_id: number): Promise<TrainingEntry> {
		try {
			const sql =
				'DELETE FROM training_entries WHERE id=($1) AND user_id=($2) RETURNING *;';
			const conn: PoolClient = await client.connect();
...
export function validateTrainingEntry(entry: TrainingEntry) {
	const entrySchema = object({
		...
		practice_type: string()
			...
			.oneOf(
				['solo', 'partnered'],
				'Practice type must be either solo or partnered'
			),
		session_duration: number()
			.required()
